...
汇总信息：使用率 温度  风扇转速，包括系统占用，用户使用和闲置。
...
进程内存监控。继续优化进程合并和显示相关的算法：目标，统计 用户下载的应用，系统备忘录/终端/预览等各种app。
...
提供当前使用app在大量使用网络（仅显示大于 200kb/s 且 前8个应用）的列表。
wifi统计信息
本地ip显示
...
电池模块无须提供电源类型，容量 剩余时间等。只需要一个控制功能。显示健康度，然后控制电池充电的最小和最大限制。
